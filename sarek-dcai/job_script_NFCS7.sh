#!/bin/bash
#SBATCH --job-name=sarek_bwa_cpucluster2
#SBATCH --output=sarek_bwa_cpucluster2_%j.out
#SBATCH --error=sarek_bwa_cpucluster2_%j.err
...
cd /path/to/project/runs2/sarek_bwa_cpucluster2 || exit 1
...
nextflow run -config /path/to/project/config/cpu.config /path/to/project/src/sarek -offline \
   ...
   --snpeff_cache '.' \
   --vep_cache '.'\
   --outdir /path/to/project/runs2/sarek_bwa_cpucluster2/results
