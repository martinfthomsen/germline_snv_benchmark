#!/bin/bash
#SBATCH --job-name=sarek_bwa_gpucluster3
#SBATCH --output=sarek_bwa_gpucluster3_%j.out
#SBATCH --error=sarek_bwa_gpucluster3_%j.err
...
cd /path/to/project/runs2/sarek_bwa_gpucluster3 || exit 1
...
nextflow run -config /path/to/project/config/gpu.config /path/to/project/src/sarek -offline \
   ...
   --snpeff_cache '.' \
   --vep_cache '.'\
   --outdir /path/to/project/runs2/sarek_bwa_gpucluster3/results
