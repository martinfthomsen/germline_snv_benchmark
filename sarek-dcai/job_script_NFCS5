#!/bin/bash
#SBATCH --job-name=sarek_pbfq2bam
#SBATCH --output=sarek_pbfq2bam_%j.out
#SBATCH --error=sarek_pbfq2bam_%j.err
...
cd /path/to/project/runs2/sarek_pbfq2bam || exit 1
...
nextflow run -config /path/to/project/config/gpu.config /path/to/project/src/sarek -offline \
   ...
   --snpeff_cache '.' \
   --vep_cache '.'\
   --outdir /path/to/project/runs2/sarek_pbfq2bam/results
