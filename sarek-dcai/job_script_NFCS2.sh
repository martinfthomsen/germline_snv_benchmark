#!/bin/bash
#SBATCH --job-name=sarek_bwa_gpucluster2
#SBATCH --output=sarek_bwa_gpucluster2_%j.out
#SBATCH --error=sarek_bwa_gpucluster2_%j.err
...
cd /path/to/project/runs2/sarek_bwa_gpucluster2 || exit 1

ml load Nextflow/25.10.2

export NXF_OFFLINE=true

nextflow run -config /path/to/project/config/gpu.config /path/to/project/src/sarek -offline -resume \
   ...
   --snpeff_cache '.' \
   --vep_cache '.'\
   --outdir /path/to/project/runs2/sarek_bwa_gpucluster2/results
